...
from DBLP.graph import create_heterograph


def create_heterograph1():
    data_dict = {
...
def hin2vec_embedding(g, metapaths, walk_length=1, num_walks=10, vector_size=32, window=1, min_count=0, sg=1):
    walks = []
    ...
    for ntype in g.ntypes:
        ...
        # print(f"Testing nodes of type {ntype}")
        for node in nodes:
            node_id = node.item()  # 转换为整数
            for _ in range(num_walks):
                walk = metapath_random_walk(g, node_id, ntype, metapaths, walk_length)
                # print(f"Walk: {walk}")
                if len(walk) > 1:  # 确保生成的walks长度大于1，表示有跳转
                    walks.append(walk)
                # print(processed_walks, walk)
                processed_walks += 1
...
node_embeddings = hin2vec_embedding(g, metapaths, walk_length=10, num_walks=10)  # 使用较小的walk_length和num_walks进行测试
...
# 输出某个节点类型的嵌入
print(g.nodes['author'].data['feat'].shape,g.nodes['author'].data['feat'])
print(g.nodes['paper'].data['feat'].shape,g.nodes['paper'].data['feat'])
print(g.nodes['venue'].data['feat'].shape,g.nodes['venue'].data['feat'])
